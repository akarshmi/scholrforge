...
import { CheckCircle2, Github, Scale, Cpu, Tag } from 'lucide-react'
...
export default function Step6Review() {
  ...
  const diff = difficultyConfig[v.difficulty] ?? difficultyConfig.beginner
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass rounded-xl overflow-hidden">
        <div className="h-px" style={{ background: 'linear-gradient(to right, rgba(108,99,255,0.7), rgba(0,212,170,0.7), transparent)' }} />

        <div className="divide-y divide-border/30">
          {/* Title + meta */}
          <div className="p-4 space-y-2">
            <h3 className="font-semibold text-foreground">{v.title || <span className="text-muted-foreground/40">No title</span>}</h3>
      ...
      <div className="space-y-4">
        <FormField control={form.control} name="repositoryUrl" render={({ field, fieldState }) => (
          <FormItem>
            <label className={labelCls}>
              <span className="flex items-center gap-1.5"><Github className="w-3 h-3" /> Repository URL (optional)</span>
            </label>
            <FormControl>
      ...
      <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl border"
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Your project will be published and immediately visible to the community.
        </p>
      </div>
    </div>
  )
