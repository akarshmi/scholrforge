...
export default function Step3TechStack() {
  ...
  // newTechStackNames — matches backend Set<String> newTechStackNames
  const selected: string[] = form.watch('newTechStackNames') ?? []
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
        {filtered.length === 0 ? (
          <p className="text-sm text-muted-foreground/50 text-center py-6">No technologies found</p>
        ) : (
          filtered.map(tech => {
            ...
            return (
              ...
              >
                <Cpu
                  ...
                  style={{ color: isSelected ? '#6c63ff' : 'rgba(255,255,255,0.25)' }}
                />
                <span
                ...
                {isSelected && <CheckCircle2 className="w-4 h-4 shrink-0" style={{ color: '#6c63ff' }} />}
              </button>
            )
          })
        )}
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        {selected.length > 0 && (
          <button
            type="button"
            onClick={() => form.setValue('newTechStackNames', [], { shouldValidate: true })}
      ...
      {form.formState.errors.newTechStackNames && (
        <FormMessage className="text-xs">
          {String(form.formState.errors.newTechStackNames.message)}
        </FormMessage>
      )}
    </div>
  )
